...
- Application 设置维护仓库地址、精确分支和代码子目录的多条绑定；新迁移 `0028_repository_bindings`，旧应用默认无绑定。
- 本地 MCP 根据 Git 上下文精确匹配有发布权限的应用；构建命令与产物/版本来源留在代码目录的 `artifact-center.release.json`。
- 构建后预览目标和制品摘要，用户确认一次后上传快照并核对服务器结果；`markLatest:false`，不自动替换最新版本。
- 新 MCP 上传工具改为 `planId` + `confirmed`，需更新本机客户端；见 [ADR-0023](docs/adr/0023-repository-publishing-preview.md)。
